...
export function requiredSafeSigningSteps(inputs, env = process.env) {
  ...
  const agentSteps = inputs.agentEvidenceMode === "verified-reuse" ? [
    command("installed-agent", "node", [
      "scripts/release/safe-signing-agent-evidence.mjs",
      "--candidate", inputs.candidate, "--app", inputs.app,
      "--certification", inputs.reuseAgentCertification, "--campaign", inputs.reuseAgentCampaign,
      "--runtime-output", inputs.runtime, "--agent-gates-output", inputs.agentGates,
      "--report", inputs.agentImport, "--executor", reliabilityVerifier,
      "--ui-driver", reliabilityUiDriver,
      ...uiDriverDependencies.flatMap((path) => ["--ui-driver-dependency", path]),
      "--installed-ui-driver", installedDriver,
      "--installed-ui-driver-dependency", "scripts/product/drivers/macos-installed-agent-ui-wait.mjs",
    ]),
  ] : [
...
export function candidateInputs(parsed, runId) {
  ...
  return {
    ...
    agentImport: resolve(join(runRoot, "agent-evidence-import.json")),
